#!/usr/bin/env python3
"""
Pitchfork Experimental – Album Reviews RSS Feed Generator
----------------------------------------------------------
Scrapes https://pitchfork.com/genre/experimental/
filters for URLs containing "reviews/albums", and writes feed.xml.
...
FEED_URL = f"{BASE_URL}/genre/experimental/"
...
def is_album_review(url):
    """Return True only if the URL contains 'reviews/albums'."""
    return "reviews/albums" in url


def fetch_reviews():
    """Fetch the Pitchfork experimental genre page and parse album review items."""
    ...
    # Pitchfork is a Next.js app — all page data is in a __NEXT_DATA__ script tag
                ...
                for field in ("albumreviews", "reviews", "items"):
                    ...
                    if isinstance(items, list) and items:
                        for item in items:
                            ...
                            if review and is_album_review(review["link"]):
    ...
    # Fallback: scan all <a> tags on the page for album review links
    if not reviews:
        print("  Falling back to HTML link scraping...")
        seen = set()
        for a_tag in soup.find_all("a", href=True):
            href = a_tag["href"]
            full_url = BASE_URL + href if href.startswith("/") else href

            if not is_album_review(full_url) or full_url in seen:
                continue
            seen.add(full_url)

            # Walk up the DOM to find the nearest card container for metadata
    ...
    print(f"  Found {len(reviews)} album reviews.")
    return reviews
...
def build_rss(reviews):
    ...
    SubElement(channel, "title").text = "Pitchfork – Experimental Album Reviews"
    SubElement(channel, "link").text = FEED_URL
    SubElement(channel, "description").text = (
        "Latest experimental album reviews from Pitchfork."
    )
...
if __name__ == "__main__":
    ...
    if not reviews:
        print("No album reviews found. The page structure may have changed.", file=sys.stderr)
        sys.exit(1)
